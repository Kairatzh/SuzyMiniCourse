...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not topic:
        return await update.message.reply_text(" Введи тему курса!")

    if len(topic) > 500:
        return await update.message.reply_text(" Слишком длинный запрос (макс. 500 символов).")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if "event loop is already running" in str(e):
            logger.warning(" Используется альтернативный запуск...")
